...
def create_parser():
    ...
    parser.add_option('--sigma', default=25,
...
def main():
	...
	xds = xds_from_ms(msname,
					  ...
					  chunks={'row': options.nrows, 'chan': options.nchans},
	...
	for i, ds in enumerate(xds):
		...
		uflag = flagchisq(resid, weight, flag, use_corrs, sigma=options.sigma)

        out_ds = ds.assign(**{options.fcol: (("row", "chan", "corr"), uflag)})
        out_data.append(out_ds)

    writes = xds_to_table(out_data, msname, columns=[args.fcol]))
