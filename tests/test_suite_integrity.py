...
Most of these tests need the ``[full,web]`` extras. Without them the heavy
modules skip at *import*, which pytest reports as a single skip per module --
a lightweight run says "10 passed, 2 skipped", not the 17 tests that did not
run. CI installs the lightweight package by design, so a green CI run proves
very little, and tests could be deleted or quietly defanged without anything
noticing.
